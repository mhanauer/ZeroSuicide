...
## Get rid of year two, because we are not starting out in January and the ts object is thinking that we are starting in January 
ITSRolling = subset(ITSRolling, Year != 2)
describe.factor(ITSRolling$Year)
ITSRolling[120:145,]
...
Model 5: CRI's rolling sum
```{r}
model_lm = lm(RollingSumTN ~ Time*Intervention, data = ITSRolling)  
summary(model_lm)
...
Maybe we can compare parameter estimates.  The values should be the same.  If we have the parameter estimate and the standard error, then we compare them via a t test??? The outcome is different numerically, but the same thing.
```{r}
