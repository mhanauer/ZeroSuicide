...
Also demonstrating that adding time does not help
```{r}
...
anova(model_p_plus, model_p_time_inter)

AIC(model_p_time_inter)
BIC(model_p_time_inter)
AIC(model_p_plus)
BIC(model_p_plus)
```
...
InSuicides$Quarter = ifelse(InSuicides$MonthNum <= 3, 1, ifelse(InSuicides$MonthNum >= 4 & InSuicides$MonthNum <= 6, 2, ifelse(InSuicides$MonthNum >= 7 & InSuicides$MonthNum <= 9, 3, ifelse(InSuicides$MonthNum >= 10, 4, InSuicides$MonthNum))))
...
InSuicides$Intervention = rep(9, dim(InSuicides)[1])
...
model_in_tn = glm(Suicides ~ State + Intervention + factor(Quarter), family = "poisson", data = IN_TN_dat)
summary(model_in_tn)
exp(0.27302)
```
